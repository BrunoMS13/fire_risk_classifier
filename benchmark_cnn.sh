...
docker run -it --rm --gpus all -v "$WEIGHTS_PATH:$DOCKER_WEIGHTS_PATH" fire_risk_classifier_image poetry run train --algorithm densenet161 --batch_size 16 --train True --num_epochs 12 --num_classes $NUM_CLASSES --images_dir $IMAGE_DIR --wd 1e-4

echo "Copying densenet161..."

mkdir -p ~/models/2C/densenet161_1e4
cp -r $WEIGHTS_PATH/densenet161_body_2C.pth ~/models/2C/densenet161_1e4
cp -r $WEIGHTS_PATH/densenet161_body_2C_metrics.json ~/models/2C/densenet161_1e4
